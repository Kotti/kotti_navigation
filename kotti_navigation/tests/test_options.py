...
def dc(content, name='/tmp/out.html'):
...
class TestOptions:
    ...
    def test_hidden_nav_points(self, webtest, kn_content, assign_left):
        ...
        assert 'Document 1 1 is no longer visible' in resp.text
    ...
    def test_include_root(self, webtest, kn_content, assign_left):
        ...
        resp = webtest.get('/')
        assert 'title="Congratulations! ' not in resp.text

        set_nav_setting('left', 'options', ['include_root'])
        resp = webtest.get('/')
        assert 'title="Congratulations! ' in resp.text
    ...
    def test_stacked(self, webtest, kn_content, assign_left):
        ...
        resp = webtest.get('/')
        assert '<ul class="nav nav-pills nav-stacked">' not in resp.text

        set_nav_setting('left', 'options', ['stacked'])
        resp = webtest.get('/')
        assert '<ul class="nav nav-pills nav-stacked">' in resp.text
